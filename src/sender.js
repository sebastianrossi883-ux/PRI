...
class Sender {
  ...
  async invia(cliente, template) {
    ...
    const chatId = numeroAChatId(cliente.numero);

    if (this.dryRun) {
      log.info(`[DRY-RUN] Invierei a ${cliente.nome} (${cliente.numero}): "${testo}"`);
    ...
    try {
      ...
      log.ok(`Messaggio inviato a ${cliente.nome} (${cliente.numero})`);
      return { ok: true };
    } catch (err) {
      ...
      log.error(`Invio fallito a ${cliente.nome} (${cliente.numero}):`, err.message || err);
